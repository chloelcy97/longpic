...
$('#tabs').addEventListener('click', e => {
  ...
  $('#panel-write').hidden = b.dataset.tab !== 'write';
  $('#panel-set').hidden   = b.dataset.tab !== 'set';
});

// 示例：哈姆雷特第三幕第一场的独白。原文早已进入公有领域，
// 中文是这里自己译的 —— 现成译本另有版权。
$('#demo').addEventListener('click', () => {
  state.title = '生存，还是毁灭';
  state.body = [
    '生存，还是毁灭，问题就在这里。默默忍受命运的暴击，还是挺身而出，与无边的苦难对抗，把它们一举扫平——哪一样更高贵？',
    ...
    '你要是既贞洁又美丽，那么你的贞洁最好不要跟你的美丽来往。美貌能把贞洁变成淫荡，贞洁却未必能把美貌感化成清白。从前这是件难事，现在世道给它做了证。',
    '我从前爱过你。你不该信我的——我们这种人，纵然接上了德行的枝，也改不掉老根上的酸味。我并没有爱过你。',
    '进尼姑庵去吧。何必生一堆罪人出来？我自己还算得上规矩，可数落起来，我竟宁愿母亲没有生下我。',
  ].join('\n\n');
  el.title.value = state.title;
  el.body.value = state.body;
  schedule();
